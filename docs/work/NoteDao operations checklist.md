...
* [X] Return a single instance, selected by {_SELECT * FROM note WHERE note_id = :noteId_}

* [X] Return multiple instances, selected by {_SELECT * FROM note WHERE user_id = :userId ORDER BY created DESC_}
