-- Generated 2025-07-10 16:34:44-0600 for database version 1

CREATE TABLE IF NOT EXISTS `user` (`user_id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `oauth_key` TEXT NOT NULL, `display_name` TEXT NOT NULL COLLATE NOCASE, `created` INTEGER NOT NULL, `modified` INTEGER NOT NULL);
...
CREATE TABLE IF NOT EXISTS `task` (`task_id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `title` TEXT NOT NULL COLLATE NOCASE, `description` TEXT, `created` INTEGER NOT NULL, `modified` INTEGER NOT NULL, `due_date` INTEGER, `completed_date` INTEGER NOT NULL, `user_id` INTEGER NOT NULL, FOREIGN KEY(`user_id`) REFERENCES `user`(`user_id`) ON UPDATE NO ACTION ON DELETE CASCADE );
...
CREATE TABLE IF NOT EXISTS `reminder` (`reminder_id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `title` TEXT NOT NULL COLLATE NOCASE, `description` TEXT, `created` INTEGER NOT NULL, `modified` INTEGER NOT NULL, `selected_date` INTEGER, `completed_date` INTEGER NOT NULL, `user_id` INTEGER NOT NULL, FOREIGN KEY(`user_id`) REFERENCES `user`(`user_id`) ON UPDATE NO ACTION ON DELETE CASCADE );
